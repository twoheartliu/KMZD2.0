window.onload = function () {
    ...
    playBtn.onclick = function () {
        ...
        if (musicAudio.paused) {
            playBtn.style.backgroundImage = 'url(../../image/播放.png)';
            musicAudio.play();
        } else {
            playBtn.style.backgroundImage = 'url(../../image/暂停.png)';
            musicAudio.pause();
        }
    };
    ...
    modeBtn.onclick = function () {
        if (musicAudio.loop) {
            ...
            this.style.backgroundImage = 'url(../image/随机.png)';
        } else {
            musicAudio.loop = true;
            this.style.backgroundImage = 'url(../image/单曲循环.png)';
        }
    };
    ...
    function getMusic () {
        ajax({
            ...
            success: function (response) {
                ...
                // var recordImgs = url(widget//res/addcover03.png)
                songTitle.innerHTML = songObj.title;
                singer.innerHTML = songObj.artist;
                //这块是专辑图片暂时为空不能解释
                // recordImg.src = recordImgs.picture;
                // alert(recordImg.src);
